...
if(isset($_COOKIE['ID'])&&$_COOKIE['ID']!=""){
    ...
    if($Jg==1){ 
        ...
        if($_POST['BT']!=""&& $_POST['writer']!=""&& $_POST['content']!=""&&$_POST['ly1']!=""&&$_POST['lm']!=""&&$_FILES["myfile"]["name"]!=""){
            ...
            $ly=$_POST['ly1'];
            $sql1="update hsports set Hname='$name', Hwriter='$writer',Hconten='$conten',Horigin='$ly',FMian ='$filename'  where ID=$ID";
            $rs1=$link->query($sql1);
            $aff1=$link->affected_rows;
            if($aff1>0){
                $fc->alrt('修改热门课程文章成功','../HS-modify.php?ID='.$ID);
            }else{
                $fc->alrt('修改热门课程文章失败','../HS-xiugai.php?ID='.$ID);
            }
        }else{
            $fc->alrt('请将信息填写完整','../HS-xiugai.php?ID='.$ID);
        }
    }else{
